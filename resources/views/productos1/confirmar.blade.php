...
@section('contenido')
    <div class="container">
        ...
        <form method="POST" action="{{ route('productos1.destroy', $productos->idproducto) }}">
            @method('delete')
            @csrf
            <button type="submit" class="btn btn-danger"><i class="fas facheck-square"></i> SI</button>
            <a href="{{ route('cancelar_prod1') }}" class="btn btnprimary"><i class="fas fa-times-circle"></i> NO</button></a>
        </form>
    @endsection
